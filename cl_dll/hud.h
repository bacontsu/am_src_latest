...
bool LoadShaderFile(char *fname, unsigned int &idx, bool fragment = true);
float LnrIntrpltn(float a, float b, float t);
...
public:
	...
	int MsgFunc_AllowWepSw( const char *pszName, int iSize, void *pbuf );
...
public:
	...
	cvar_t	*m_pCvarDraw;
	// Shepard : FMOD stuff
	cvar_t	*m_pCvarFMODDebugShowCPUUsage;
#ifdef WIN32
	cvar_t	*m_pCvarFMODMuteWhenNotFocused;
#endif
	cvar_t	*m_pCvarFMODOutput;
	cvar_t	*m_pCvarFMODSpeaker;
...
public:
	...
	void _cdecl MsgFunc_AddRope(const char *pszName, int iSize, void *pbuf);
