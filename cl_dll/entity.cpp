...
// p_render
int g_flashlight[MAX_PLAYERS+1];
...
int DLLEXPORT HUD_AddEntity( int type, struct cl_entity_s *ent, const char *modelname )
{
	...
	// p_render
	if (!GlowFilterEntities( type, ent, modelname ))
		return 0;

	if (!CubemapFilterEntities(type, ent))
		return 0;
...
void DLLEXPORT HUD_TxferLocalOverrides( struct entity_state_s *state, const struct clientdata_s *client )
{
	...
	RendererUpdateEntityList(); // p_render
}
...
void DLLEXPORT HUD_ProcessPlayerState( struct entity_state_s *dst, const struct entity_state_s *src )
{
	...
	{
		g_flashlight[dst->number] = 1;
	}
	else
		g_flashlight[dst->number] = 0;
	// p_render
}
...
//hawk
float VectorLenght(vec3_t in)
{
	float sqrLen = (in.x*in.x) +
		(in.y*in.y) +
		(in.z*in.z);
...
void DLLEXPORT HUD_TxferPredictionData ( struct entity_state_s *ps, const struct entity_state_s *pps, struct clientdata_s *pcd, const struct clientdata_s *ppcd, struct weapon_data_s *wd, const struct weapon_data_s *pwd )
{
	...
	velLen = VectorLenght(ppcd->velocity);//hawk
...
*/
	...
	RendererCreateEntities(); // p_render
}
...
void DLLEXPORT HUD_StudioEvent( const struct mstudioevent_s *event, const struct cl_entity_s *entity )
{
	...
	//SYS ADD: this fixes wrong settings on weapon muzzleflashes.
	int iMuzzleSize = atoi(event->options);
	...
	{
	case 5001:
		if ( entity->curstate.body != 1 ) // Shepard : No muzzleflashes if the weapon is silenced
		{
			gEngfuncs.pEfxAPI->R_MuzzleFlash((float *)&entity->attachment[0], iMuzzleSize);
			gParticleEngine.CreateCluster( "weapons/muzzle_smoke.txt", (float *)&entity->attachment[0], (float *)&entity->angles.Normalize(), 0 );
		//	gEngfuncs.Con_Printf("angles :%f \n", (float *)&entity->angles.Normalize());
		//	gEngfuncs.Con_Printf("%f %f %f\n", (float *)&entity->angles[0], (float *)&entity->angles[1], (float *)&entity->angles[2]);

			DlightFlash( (float *)&entity->attachment[0] );
		}
		break;
	case 5011:
		gEngfuncs.pEfxAPI->R_MuzzleFlash((float *)&entity->attachment[1], iMuzzleSize);
		DlightFlash( (float *)&entity->attachment[1] );
		break;
	case 5021:
		gEngfuncs.pEfxAPI->R_MuzzleFlash((float *)&entity->attachment[2], iMuzzleSize);
		DlightFlash( (float *)&entity->attachment[2] );
		break;
	case 5031:
		gEngfuncs.pEfxAPI->R_MuzzleFlash((float *)&entity->attachment[3], iMuzzleSize);
		DlightFlash( (float *)&entity->attachment[3] );
		break;
	case 5002:
		gEngfuncs.pEfxAPI->R_SparkEffect((float *)&entity->attachment[0], iMuzzleSize, -100, 100);
		break;
	// Client side sound
	case 5004:
		gFMOD.Play3DSoundAtPosition( (float *)&entity->attachment[0], (char *)event->options, 2048.0f, -1.0f, entity->index );
		break;
	default:
		break;
	}
	...
	void	( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	while ( pTemp )
	{
		...
		pTemp->entity.curstate.colormap = 1; // p_render
