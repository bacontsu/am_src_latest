...
int SENTENCEG_PlayRndSz(edict_t *entity, const char *szgroupname,
	float volume, float attenuation, int flags, int pitch, bool bRadio)
{
	char name[64];
	int ipick;
	int isentenceg;
	...
	if (isentenceg < 0)
	{
		ALERT(at_console, "No se encuentra el siguiente grupo de sentencia %s\n", szgroupname);
		return -1;
	}
	...
	{
		//	if ( bRadio == TRUE )
		//	EMIT_SOUND_DYN(ENT(0), CHAN_STATIC, name, 1.0, ATTN_NONE, 0, PITCH_NORM);
		//	else
		EMIT_SOUND_DYN(entity, CHAN_VOICE, name, volume, attenuation, flags, pitch);
	}
...
void SENTENCEG_Init()
{
	...
	//	byte *pMemFile = g_engfuncs.pfnLoadFileForMe( "scripts/sentences.txt", &fileSize );
						   ...
						   int flags, int pitch)
{
	...
	if (sample && *sample == '!')
	{
		sentence = SENTENCEG_Lookup(sample, name);
		if (sentence >= 0){
			ALERT(at_console, "Sentence sound %s\n", name);
			EMIT_SOUND_DYN2(entity, channel, name, volume, attenuation, flags, pitch);
		}
		else
			ALERT( at_console, "Unable to find %s in sentences.txt\n", sample );
	}
	else
 ...
 /*	CBaseEntity *pEntity = CBaseEntity::Instance(entity);
	if (pEntity){
		if (sentence >= 0)
			UTIL_ShowMessagePVS(name, pEntity->pev->origin);
		else
			UTIL_ShowMessagePVS(sample, pEntity->pev->origin);
	} */
}
...
extern int gmsgFMOD2D;
extern int gmsgFMOD3D;

void EMIT_SOUND_FMOD_2D_ONE(edict_t *tEntity, const char *cSample, float flPitch, int iLookUpSubtitles )
{
	//ALERT(at_console, "EMIT_SOUND_FMOD_2D_ONE iLookUpSubtitles = %i\n", iLookUpSubtitles);
	if ( cSample && *cSample == '!' )
	{
		char cName[32];
		if ( SENTENCEG_Lookup( cSample, cName ) >= 0 )
		{
			MESSAGE_BEGIN( MSG_ONE, gmsgFMOD2D, NULL, tEntity );
				WRITE_STRING( cName );
				WRITE_COORD( flPitch );
			MESSAGE_END();
		}
	...
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgFMOD2D, NULL, tEntity );
			WRITE_STRING( cSample );
			WRITE_COORD( flPitch );
		MESSAGE_END();
	}

	CBaseEntity *pEntity = CBaseEntity::Instance(tEntity);
	if (pEntity)
	{
	//	ALERT(at_console, "EMIT_SOUND_FMOD_2D_ONE iLookUpSubtitles = %i\n", iLookUpSubtitles);
		if (iLookUpSubtitles == 1 && CVAR_GET_FLOAT("hud_draw") == 1)
		UTIL_ShowMessagePVS(cSample, pEntity->pev->origin);
	}
}

void EMIT_SOUND_FMOD_2D_ALL( const char *cSample, float flPitch )
{
	if ( cSample && *cSample == '!' )
	{
		char cName[32];
		if ( SENTENCEG_Lookup( cSample, cName ) >= 0 )
		{
			MESSAGE_BEGIN( MSG_ALL, gmsgFMOD2D );
				WRITE_STRING( cName );
				WRITE_COORD( flPitch );
			MESSAGE_END();
		}
		...
		for ( int i = 1; i <= gpGlobals->maxClients; i++ )
			UTIL_ShowMessage( cSample, UTIL_PlayerByIndex( i ) );
	}
	else
	{
		MESSAGE_BEGIN( MSG_ALL, gmsgFMOD2D, NULL );
			WRITE_STRING( cSample );
			WRITE_COORD( flPitch );
		MESSAGE_END();
	}
...
void EMIT_SOUND_FMOD_3D( edict_t *tEntity, const char *cSample, float flMinDistance, float flMaxDistance, int iEntityIndexToFollow, float flPitch )
{
	if ( flMaxDistance == -1.0f )
		flMaxDistance = flMinDistance * 1.5;
	...
	if ( cSample && *cSample == '!' )
	{
		...
		if ( SENTENCEG_Lookup( cSample, cName ) >= 0 )
		{
			MESSAGE_BEGIN( MSG_ALL, gmsgFMOD3D );
				WRITE_COORD( VARS( tEntity )->origin.x );
				WRITE_COORD( VARS( tEntity )->origin.y );
				WRITE_COORD( VARS( tEntity )->origin.z );
				WRITE_STRING( cSample );
				WRITE_COORD( flMinDistance );
				WRITE_COORD( flMaxDistance );
				WRITE_LONG( iEntityIndexToFollow );
				WRITE_COORD( flPitch );
			MESSAGE_END();
		}
		else
...
char TEXTURETYPE_FindTrace(TraceResult *ptr, Vector vecSrc, Vector vecEnd)
{
	char chTextureType;
	char szbuffer[64];
	const char *pTextureName;
