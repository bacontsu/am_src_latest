...
void CFuncCollideWall::Spawn(void)
{
	...
	SetThink(&CFuncCollideWall::MyThink);
...
void CFuncCollideWall::MyThink(void)
{
...
void CFuncCollideWall::Precache(void)
{
	PRECACHE_SOUND("weapons/bulletimpact/water1.wav");
	PRECACHE_SOUND("weapons/bulletimpact/water2.wav");
	PRECACHE_SOUND("weapons/bulletimpact/water3.wav");

	PRECACHE_SOUND("weapons/bulletimpact/gascan1.wav");
	PRECACHE_SOUND("weapons/bulletimpact/gascan2.wav");
}
...
int CFuncCollideWall::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	...
	if (pev->frags == 9) //leaves
	{
		switch (RANDOM_LONG(0, 2))
		{
		case 0: UTIL_EmitAmbientSound(ENT(0), trace.vecEndPos, "AMBIENCE/BUSH1.wav", flVolume, ATTN_NORM, 0, 100); break;
		case 1: UTIL_EmitAmbientSound(ENT(0), trace.vecEndPos, "AMBIENCE/BUSH2.wav", flVolume, ATTN_NORM, 0, 100); break;
		case 2: UTIL_EmitAmbientSound(ENT(0), trace.vecEndPos, "AMBIENCE/BUSH3.wav", flVolume, ATTN_NORM, 0, 100); break;
		}
	}
	else
	{
	...
	else if (pev->frags == 8) //leaves
		WRITE_SHORT(iDefaultLeaves);
	else if (pev->frags == 9) //bush leaves
		WRITE_SHORT(iDefaultLeavesBush);
	else
