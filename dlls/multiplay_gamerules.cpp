...
// override some values for multiplay.
	...
	// Knife whack (stab)
	gSkillData.plrDmgKnifeStab = 50;

	// Fists
	gSkillData.plrDmgFists = 20;

	// Semtex charge
	gSkillData.plrDmgSemtex = 120;

	// IMI Desert Eagle .50 Round
	gSkillData.plrDmgDeagle = 25;

	// Colt M1911A2 45 ACP Round
	gSkillData.plrDmg1911 = 18;

	// Beretta 92 FS 9mm Round
	gSkillData.plrDmgBeretta = 20;

	// Glock 18 9mm Round
	gSkillData.plrDmgGlock = 16;

	// Colt Anaconda (Python) .44 Round
	gSkillData.plrDmgAnaconda = 55;

	// H&K MP5 9mm Round
	gSkillData.plrDmgMP5 = 12;

	// FN P90 .57 Round
	gSkillData.plrDmgP90 = 16;

	// PP-19 Bizon 9mm Round
	gSkillData.plrDmgBizon = 7;

	// FAMAS G2 5.56 Round
	gSkillData.plrDmgFAMAS = 20;

	// Colt M4A1 5.56 Round
	gSkillData.plrDmgM4A1 = 25;

	// AK47 7.62 Round
	gSkillData.plrDmgAK47 = 28;

	// M249 5.56 Round
	gSkillData.plrDmgM249 = 25;

	// SPAS 12 Round
	gSkillData.plrDmgSPAS12 = 20;

	// Less Lethal shotgun Round
	gSkillData.plrDmgLessLethal = 1;

	// USAS-12 Round
	gSkillData.plrDmgUSAS = 22;

	// M14 7.62 Round
	gSkillData.plrDmgM14 = 65;

	// L96A1 7.62 Round
	gSkillData.plrDmgL96A1 = 120;
...
BOOL CHalfLifeMultiplay::FShouldSwitchWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pWeapon )
{
	...
	if ( !pPlayer->m_pActiveItem->CanHolster() || pPlayer->m_flEpipenTime != -1.0f || pPlayer->m_flGrenadeTime != -1.0f )
	{
...
BOOL CHalfLifeMultiplay :: GetNextBestWeapon( CBasePlayer *pPlayer, CBasePlayerItem *pCurrentWeapon )
{
	...
	if ( !pCurrentWeapon->CanHolster() || pPlayer->m_flEpipenTime != -1.0f || pPlayer->m_flGrenadeTime != -1.0f )
	{
	...
	// at least get the knife, but ya never know.
	if ( !pBest )
	{
		return FALSE;
	}
...
void CHalfLifeMultiplay :: InitHUD( CBasePlayer *pl )
{
	...
	// Shepard : To avoid music being repeated when respawning
	pl->m_flMusicInitTime = gpGlobals->time + 2.0f;
}
...
void CHalfLifeMultiplay :: PlayerSpawn( CBasePlayer *pPlayer )
{
	...
	if ( addDefault )
	{
		...
		pPlayer->GiveAmmo( 80, "9mm", _9MM_MAX_CARRY );// 4 full reloads
	}
}
...
#ifndef HLDEMO_BUILD
	if ( pVictim->HasNamedPlayerItem( "weapon_semtex" ) || pVictim->HasNamedPlayerItem( "weapon_satchel" ) )
	{
...
void CHalfLifeMultiplay :: ChangeLevel( void )
{
	...
	strcpy( szFirstMapInList, "hldm1" );  // the fabsolute default level is hldm1
