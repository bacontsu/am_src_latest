...
#define MAX_STAMINA			(gpGlobals->time + 30.0)
...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] =
{
	...
	// Shepard : FMOD stuff
	DEFINE_FIELD(CBasePlayer, m_bMusicIsAction, FIELD_BOOLEAN),
	DEFINE_FIELD(CBasePlayer, m_bMusicIsAllowedToSwitchToAction, FIELD_BOOLEAN),
	DEFINE_FIELD(CBasePlayer, m_flMusicBackToStealthTime, FIELD_TIME),
	DEFINE_FIELD(CBasePlayer, m_iLastReverberationType, FIELD_INTEGER),

	// Shepard : Sprint stuff
	DEFINE_FIELD(CBasePlayer, m_flNextSprintSoundUpdate, FIELD_TIME),
	DEFINE_FIELD(CBasePlayer, m_flNextStaminaUpdate, FIELD_TIME),
	DEFINE_FIELD(CBasePlayer, m_flPlayerClientSpeed, FIELD_FLOAT),
	DEFINE_FIELD(CBasePlayer, m_flPlayerSpeed, FIELD_FLOAT),
	DEFINE_FIELD(CBasePlayer, m_flStaminaOut, FIELD_TIME),

	DEFINE_FIELD(CBasePlayer, _r, FIELD_INTEGER),
	DEFINE_FIELD(CBasePlayer, _g, FIELD_INTEGER),
	DEFINE_FIELD(CBasePlayer, _b, FIELD_INTEGER),
	DEFINE_FIELD(CBasePlayer, _s, FIELD_INTEGER),
	DEFINE_FIELD(CBasePlayer, _t, FIELD_FLOAT),

	DEFINE_FIELD(CBasePlayer, m_flEpipenTime, FIELD_TIME),
	DEFINE_FIELD(CBasePlayer, m_flGrenadeTime, FIELD_TIME)
};
...
// Shepard : FMOD stuff
int gmsgFMODMusicAction = 0;
int gmsgFMODMusicStealth = 0;
int gmsgFMODReverb = 0;
int gmsgFMODStop = 0;
int gmsgFMODSwitch = 0;
int gmsgFMOD2D = 0;
int gmsgFMOD3D = 0;

int gmsgShepardEasterEgg = 0;
...
void LinkUserMessages( void )
{
	...
	// Shepard : FMOD stuff
	gmsgFMODMusicAction = REG_USER_MSG( "FMODMusicA", -1 );
	gmsgFMODMusicStealth = REG_USER_MSG( "FMODMusicS", -1 );
	gmsgFMODReverb = REG_USER_MSG( "FMODReverb", 1 );
	gmsgFMODStop = REG_USER_MSG( "FMODStop", 1 );
	gmsgFMODSwitch = REG_USER_MSG( "FMODSwitch", 1 );
	gmsgFMOD2D = REG_USER_MSG( "FMOD2D", -1 );
	gmsgFMOD3D = REG_USER_MSG( "FMOD3D", -1 );

	gmsgShepardEasterEgg = REG_USER_MSG( "ShepardEE", 1 );
...
int CBasePlayer :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if (IsAlive() && (pev->health < 35))
		flDamage *= 0.5;

	if (IsAlive() && (pev->health < 10))
		flDamage *= 0.15;
	...
	if (!m_fHurted)
	{
		...
		{
			client_blurAlpha = client_blurStrength = -1;
			MESSAGE_BEGIN(MSG_ONE, gmsgSpecMsg, NULL, pev);
			WRITE_BYTE(CL_BLURSTATE);
			WRITE_COORD(2.0);
			WRITE_COORD(0.7);
			WRITE_COORD(0.001);//immediately
			MESSAGE_END();

			blurAlpha = blurStrength = 0;
			blurFadetime = 0.01;

			//esta herido tambien
			m_fHurted = TRUE;

		//	SetThink(&CBasePlayer::RestoreSpeedVol);
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
	...
	{
		m_bMusicIsAllowedToSwitchToAction = FALSE;
		MESSAGE_BEGIN( MSG_ONE, gmsgFMODSwitch, NULL, ENT( pev ) );
			WRITE_BYTE( 0 );
		MESSAGE_END();
		MESSAGE_BEGIN( MSG_ONE, gmsgFMODMusicStealth, NULL, ENT( pev ) );
			WRITE_STRING( "death.ogg" );
			WRITE_BYTE( 1 );
		MESSAGE_END();
	}
...
void CBasePlayer::TabulateAmmo()
{
	...
	ammo_50 = AmmoInventory( GetAmmoIndex( "50" ) );
	ammo_45 = AmmoInventory( GetAmmoIndex( "45" ) );
	ammo_44 = AmmoInventory( GetAmmoIndex( "44" ) );
	ammo_57 = AmmoInventory( GetAmmoIndex( "57" ) );
...
{
//	ALERT(at_console, "PlayerDeathThink\n");
...
// choose to respawn.
	...
	// Shepard : Enable my easter egg ^^
/*	if ( !fAnyButtonDown && !g_pGameRules->IsMultiplayer() )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgShepardEasterEgg, NULL, pev );
			WRITE_BYTE( 1 );
		MESSAGE_END();
	}*/
...
void CBasePlayer::PlayerUse ( void )
{
	...
	while ((pObject = UTIL_FindEntityInSphere( pObject, pev->origin, PLAYER_SEARCH_RADIUS )) != NULL)
	{
		...
		if (pObject->ObjectCaps() & (FCAP_IMPULSE_USE | FCAP_CONTINUOUS_USE | FCAP_ONOFF_USE))
		{
			...
			if (flDot < 0.4)
			{
				...
				ALERT(at_console, "player did turn away, cancelling...\n");
	...
	if (pObject )
	{
		...
		if ( m_afButtonPressed & IN_USE )
			EMIT_SOUND_FMOD_2D_ONE( ENT( pev ), "common/wpn_select.wav" );
			 ...
			 ( (m_afButtonPressed & IN_USE) && (caps & (FCAP_IMPULSE_USE|FCAP_ONOFF_USE)) ) )
		{
			...
			if ((!strcmp((char *)STRING(pObject->pev->classname), "monster_terrorist")))
			{
				float fLootTime = 2.0f;
				#define ITEMS_TO_LOOT	3

				if (pObject->pev->deadflag == DEAD_DEAD)
				if (pObject->pev->armortype != 1)
				{
					if (!m_bLooting)
					{
					//	ALERT(at_console, "%s : %f\n", STRING(pObject->pev->classname), flDot);
					...
					if (m_bLooting)
						if ((m_flStartTime + fLootTime) <= gpGlobals->time) // Check if it has to be enabled
						{
						//	ALERT(at_console, "looted for 1.5 secs!\n");
							...
							for (int i = 0; i < RANDOM_LONG(1,ITEMS_TO_LOOT); i++)
							Loot("monster_terrorist");

							//armor type is used to let the NPC now is not lootable anymore
							pObject->pev->armortype = 1;
						}
				}
			}
		}
	...
	{
		if ( m_afButtonPressed & IN_USE )
			EMIT_SOUND_FMOD_2D_ONE( ENT( pev ), "common/wpn_denyselect.wav" );
	}
}
...
#ifdef DEBUG
extern cvar_t am_infinite_stamina;
#endif
...
void CBasePlayer::PreThink(void)
{
	...
	//	m_afPhysicsFlags |= FL_DUCKING;
		...
		//	m_afPhysicsFlags &= ~FL_DUCKING;
			...
			m_bLooting = FALSE;
//			ALERT(at_console, "button released\n");
	...
	else
	{
		...
		if (pObject)
		{
			if ((!strcmp((char *)STRING(pObject->pev->classname), "monster_terrorist")))
			{
				ClientPrint(pev, HUD_PRINTCENTER, "#LootHoldKey");

 				{
					char szText[201];
					hudtextparms_t     hText;
					sprintf(szText, "Soldier %s", STRING(pObject->pev->noise3));
	...
	// Shepard : Update stamina
	if ( m_flNextStaminaUpdate < gpGlobals->time )
	{
		//ALERT( at_console, "[Sprint] Current = %f --- No stamina = %f --- Diff = %f\n", gpGlobals->time, m_flStaminaOut, (m_flStaminaOut - gpGlobals->time) );
		if ( pev->button & IN_SPRINT && pev->waterlevel == 0 ) // Player is holding the sprint button and he's not on/in/under water
		{
			float flMovingVelocity = sqrt( (pev->velocity.x * pev->velocity.x) + (pev->velocity.y * pev->velocity.y) + (pev->velocity.z * pev->velocity.z) );
			if ( (pev->button & (IN_FORWARD | IN_BACK)) && (flMovingVelocity > 130) ) // Sprint only works for forward and backward movement and requires a "start up"
			{
				float flStaminaLeft = m_flStaminaOut - gpGlobals->time;
				if ( gpGlobals->time >= m_flStaminaOut ) // No more stamina, normal speed
				{
					m_flStaminaOut = gpGlobals->time;
					SetSpeed( 230 );
					CVAR_SET_FLOAT("cl_bobcycle", 0.8);
				}
				else if (flStaminaLeft <= 15.0) // Less than half of stamina, slower sprint
				{
					SetSpeed(300);
					CVAR_SET_FLOAT("cl_bobcycle", 0.5);
				}
				else // More than half of stamina, fast sprint
				{
					SetSpeed(400);
					CVAR_SET_FLOAT("cl_bobcycle", 0.3);
				}

#ifdef DEBUG
				if ( am_infinite_stamina.value == 1 )
					m_flStaminaOut = MAX_STAMINA;
#endif

				if ( gpGlobals->time >= m_flNextSprintSoundUpdate )
				{
					EMIT_SOUND_FMOD_3D( ENT( pev ), "player/damage/breath_heart.wav", 512.0f, -1.0f, entindex() );
					m_flNextSprintSoundUpdate = gpGlobals->time + 8.5;
				}
			}
		}
		else // Not sprinting, replenish stamina if we have to and reset the speed
		{
			if ( m_flStaminaOut >= MAX_STAMINA ) // Don't go over 100%
				m_flStaminaOut = MAX_STAMINA;
			else
				m_flStaminaOut += 0.05;
		...
		// 100fps = 0.01ms. 40fps = 1/40 = 0.025ms, use it.
		m_flNextStaminaUpdate = gpGlobals->time + 0.04;
	}
	if ( m_flPlayerSpeed != m_flPlayerClientSpeed )
	{
		CVAR_SET_FLOAT( "sv_maxspeed", m_flPlayerSpeed );
		CVAR_SET_FLOAT( "cl_forwardspeed", m_flPlayerSpeed );
		CVAR_SET_FLOAT( "cl_backspeed", m_flPlayerSpeed );
		CVAR_SET_FLOAT( "cl_sidespeed", m_flPlayerSpeed );
		g_engfuncs.pfnSetClientMaxspeed( ENT( pev ), m_flPlayerSpeed );
		m_flPlayerClientSpeed = m_flPlayerSpeed;
	}

	// Shepard : Set underwater reverberation
	if ( (pev->waterlevel == 3) && FBitSet( pev->flags, FL_INWATER ) )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgFMODReverb, NULL, pev );
			WRITE_BYTE( 23 );
		MESSAGE_END();
	}
	else if ( (pev->waterlevel != 3) && FBitSet(pev->flags, FL_INWATER) )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgFMODReverb, NULL, pev );
			WRITE_BYTE( m_iLastReverberationType );
		MESSAGE_END();
	}
	...
	if (!g_pGameRules->IsMultiplayer())//this cause lag on mp EDIT: Am it's not MP anymore, so don't take care
	{
		if (IsAlive() && (pev->health < 25) && (pev->waterlevel != 3))//valor max.: 1.25 | valor min sin fix: 0.05 | valor min. con fix: 0.2
		{
			...
			{
				EMIT_SOUND_FMOD_2D_ONE( ENT( pev ), "player/damage/corazon.wav" );
				...
				if (pev->health < 40)//try to match skills
					pev->health += 5;
			}
		}
	}
	...
	// Shepard : FMOD stuff
	//ALERT( at_console, "FMOD - Current time = %2f, Update at = %2f, Mode = %s\n", gpGlobals->time, m_flMusicBackToStealthTime, (m_bMusicIsAction) ? "ACTION" : "STEALTH" );
	//ALERT( at_console, "FMOD - First play at = %f, Update at = %2f, Time = %2f\n", m_flMusicInitTime, m_flMusicBackToStealthTime, gpGlobals->time );
	// There is no music, play the stealth music
	if ( (gpGlobals->time > m_flMusicInitTime) && (m_flMusicInitTime != -1.0f) )
	{
		char *cMusicAction;
		char *cMusicStealth;
	...
	//	ALERT(at_console, "FMOD - pEntity m_szMusicStealth  \n", pEntity->m_szMusicStealth);

		bool newGameMusicEntity = TRUE;
		...
		if (pEntity)
		{
			if (FStringNull(pEntity->m_szMusicStealth) && FStringNull(pEntity->m_szMusicAction))
				newGameMusicEntity = FALSE;

			if (newGameMusicEntity)
			{			
				cMusicAction = (char *)STRING(pEntity->m_szMusicAction);
				cMusicStealth = (char *)STRING(pEntity->m_szMusicStealth);

				MESSAGE_BEGIN(MSG_ONE, gmsgFMODMusicAction, NULL, ENT(pev));
				WRITE_STRING(cMusicAction);
				WRITE_BYTE(1);
				MESSAGE_END();
				MESSAGE_BEGIN(MSG_ONE, gmsgFMODMusicStealth, NULL, ENT(pev));
				WRITE_STRING(cMusicStealth);
				WRITE_BYTE(1);
				MESSAGE_END();
				MESSAGE_BEGIN(MSG_ONE, gmsgFMODSwitch, NULL, ENT(pev));
				WRITE_BYTE(m_bMusicIsAction ? 1 : 0);
				MESSAGE_END();
			}
			else
			{
				char szMp3FileAction[64];
				char szMp3FileStealth[64];

				sprintf(szMp3FileAction, "%s_dyn.mp3", map_name);
				sprintf(szMp3FileStealth, "%s.mp3", map_name);

				MESSAGE_BEGIN(MSG_ONE, gmsgFMODMusicAction, NULL, ENT(pev));
				WRITE_STRING(szMp3FileAction);
				WRITE_BYTE(1);
				MESSAGE_END();
				MESSAGE_BEGIN(MSG_ONE, gmsgFMODMusicStealth, NULL, ENT(pev));
				WRITE_STRING(szMp3FileStealth);
				WRITE_BYTE(1);
				MESSAGE_END();
				MESSAGE_BEGIN(MSG_ONE, gmsgFMODSwitch, NULL, ENT(pev));
				WRITE_BYTE(m_bMusicIsAction ? 1 : 0);
				...
				ALERT(at_console, "szMp3FileAction = %s --- szMp3FileStealth = %s\n", szMp3FileAction, szMp3FileStealth);
			}
		}

		m_flMusicInitTime = -1.0f;
	}

	if ( m_bMusicIsAllowedToSwitchToAction && (gpGlobals->time > m_flMusicBackToStealthTime) )
	{
		m_bMusicIsAction = FALSE;
		MESSAGE_BEGIN( MSG_ONE, gmsgFMODSwitch, NULL, ENT( pev ) );
			WRITE_BYTE( 0 );
		MESSAGE_END();
	}
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		{// after this point, we start doing damage
			...
			{//splat
				...
				EMIT_SOUND_FMOD_3D( ENT( pev ), "common/bodysplat.wav", 512.0f, -1.0f, this->entindex() );
			}
...
// Shepard : FMOD stuff
void CBasePlayer::PlayActionMusic( void )
{
	// Not allowed to do that
	if ( !m_bMusicIsAllowedToSwitchToAction )
		return;

	// Update time
	m_flMusicBackToStealthTime = gpGlobals->time + 15.0f;

	// Toggle to action if we're not already
	if ( !m_bMusicIsAction )
	{
		MESSAGE_BEGIN( MSG_ONE, gmsgFMODSwitch, NULL, ENT( pev ) );
			WRITE_BYTE( 1 );
		MESSAGE_END();
		m_bMusicIsAction = TRUE;
	}
}
...
void CBasePlayer::Spawn(void)
{
	...
	SetSpeed(230);
	CVAR_SET_FLOAT("cl_bob", 0.002);
	CVAR_SET_FLOAT("cl_bobcycle", 0.8);
	CVAR_SET_FLOAT("cl_bobup", 0.6);
...
//	SERVER_COMMAND("hud_draw 1\n");
	...
	// Shepard : FMOD stuff
	m_bMusicIsAction = g_pGameRules->IsMultiplayer();
	m_bMusicIsAllowedToSwitchToAction = !g_pGameRules->IsMultiplayer();
	m_flMusicBackToStealthTime = gpGlobals->time + 1.0;
	m_iLastReverberationType = 0;
...
void CSprayCan::Spawn ( entvars_t *pevOwner )
{
	...
	pev->nextthink = gpGlobals->time + 0.1;
	EMIT_SOUND_FMOD_3D( ENT( pev ), "player/sprayer.wav", 512.0f, -1.0f, this->entindex() );
}
...
void CBasePlayer :: FlashlightTurnOn( void )
{
	...
	if ( (pev->weapons & (1<<WEAPON_SUIT)) )
	{
		EMIT_SOUND_FMOD_3D( ENT( pev ), SOUND_FLASHLIGHT, 512.0f, -1.0f, this->entindex() );
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	{
#ifdef DEBUG
	// Shepard : Animation report on the V_ model (used for development and debugging purposes), will likely be removed on game's release
	case 64:
		ALERT( at_console, "=== WEAPON ANIMATION REPORT ===\n\n" );
		if ( m_pActiveItem )
		{
			studiohdr_t *pStudioHDR;
			int iOldModel = pev->model;
			int iOldModelIndex = pev->modelindex;
			SET_MODEL( ENT( pev ), STRING( pev->viewmodel ) );
			...
			if ( pStudioHDR != NULL )
			{
				...
				{
					float flTime = (float)pSeqDesc[i].numframes / (float)pSeqDesc[i].fps;
					ALERT( at_console, "ANIMATION NAME = \"%s\" --- ANIMATION TIME = %f\n", pSeqDesc[i].label, flTime );
				}
			}
			else
				ALERT( at_error, "\nCRITICAL ERROR --- FAILED TO GET MODEL POINTER !!!\n" );

			pev->model = iOldModel;
			pev->modelindex = iOldModelIndex;
		}
		else
			ALERT( at_console, "\nCRITICAL ERROR --- NO ACTIVE ITEM !!!\n" );
		break;
#endif
    ...
    // Shepard : Reimplemented at SysOp's request
	// TODO : Either we leave that here or we remove it before game's release
	case 100:
		if ( FlashlightIsOn() )
			FlashlightTurnOff();
		else
	...
	case 007:
		...
		{
		case 0: ALERT( at_console, "All your hard drives are gonna be FULLY erased when you quit the game because we HATE CHEATERS LIKE YOU !\n" ); break;
		case 1: ALERT( at_console, "We placed sexy pics of our GFs at the credits, but since you decided to CHEAT, you won\'t get to see them.\n" ); break;
		case 2: ALERT( at_console, "We contacted MI6, James Bond is coming to kick your ass because he hates CHEATERS !\n" ); break;
		case 3: ALERT( at_console, "Because you CHEATED, you actually deserve to be slapped to death with a spoon.\n" ); break;
		case 4: ALERT( at_console, "This is pathetic, you just placed yourself at the top of the hall of fame of shame because of the CHEAT command you just invoked.\n" ); break;
		}
		gEvilImpulse101 = TRUE;
		GiveNamedItem( "item_suit" );
		GiveNamedItem( "item_battery" );
		GiveNamedItem( "item_epipen" );
		GiveNamedItem( "item_grenade" );
		...
		GiveNamedItem( "ammo_deagleclip" );
		GiveNamedItem( "weapon_1911" );
		GiveNamedItem( "ammo_1911clip" );
		GiveNamedItem( "weapon_ber92f" );
		GiveNamedItem( "ammo_ber92fclip" );
		GiveNamedItem( "weapon_glock18" );
		GiveNamedItem( "ammo_glock18clip" );
		GiveNamedItem( "weapon_anaconda" );
		GiveNamedItem( "ammo_44" );
		...
		GiveNamedItem( "ammo_famasclip" );
		GiveNamedItem( "weapon_m4a1" );
		GiveNamedItem( "ammo_m4a1clip" );
		GiveNamedItem( "weapon_ak47" );
		GiveNamedItem( "ammo_ak47clip" );
		GiveNamedItem( "weapon_m249" );
		GiveNamedItem( "ammo_m249clip" );
		GiveNamedItem( "weapon_spas12" );
		GiveNamedItem( "ammo_buckshot" );
		GiveNamedItem( "weapon_shotgun_non_lethal" );
		GiveNamedItem( "weapon_usas" );
		GiveNamedItem( "weapon_m14" );
		GiveNamedItem( "ammo_m14clip" );
		GiveNamedItem( "weapon_l96a1" );
		GiveNamedItem( "ammo_l96a1clip" );
...
int CBasePlayer::RemovePlayerItem( CBasePlayerItem *pItem )
{
	if (m_pActiveItem == pItem)
	{
		ResetAutoaim( );
		pItem->Holster( );
		pItem->pev->nextthink = 0; // melee may be trying to swing again, etc.
		pItem->SetThink( NULL );
		m_pActiveItem = NULL;
		pev->viewmodel = 0;
		pev->weaponmodel = 0;
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if (m_fInitHUD)
	{
		m_fInitHUD = FALSE;
		gInitHUD = FALSE;
		// Shepard : Reset music (for singleplayer), multiplayer is handled CHalfLifeMultiplay
		if ( !g_pGameRules->IsMultiplayer() )
			m_flMusicInitTime = gpGlobals->time + 2.0f;
		...
		MESSAGE_BEGIN(MSG_ONE, gmsgSpecMsg, NULL, pev);
			WRITE_BYTE(CL_FOG);
			WRITE_BYTE(pWorld->v.vuser1.x);
			WRITE_BYTE(pWorld->v.vuser1.y);
			WRITE_BYTE(pWorld->v.vuser1.z);
			WRITE_COORD(pWorld->v.fuser1);
			WRITE_COORD(pWorld->v.fuser2);
		MESSAGE_END();
	...
	if (client_blurAlpha != blurAlpha || client_blurStrength != blurStrength)
	{
		MESSAGE_BEGIN(MSG_ONE, gmsgSpecMsg, NULL, pev);
		WRITE_BYTE(CL_BLURSTATE);
		WRITE_COORD(blurStrength);//sterngth
		WRITE_COORD(blurAlpha);//alpha
		WRITE_COORD(blurFadetime);//fade time
		MESSAGE_END();
		client_blurAlpha = blurAlpha;
		client_blurStrength = blurStrength;
	}
...
const char* CBasePlayer::Loot(const char *szEnemyType)
{
	typedef struct dropsStruct
	{
		const char	*sName;
		int			iItemRarity;
	} dropsStruct_t;

	typedef struct beastStruct
	{
		const char	*sName;
		int			iDropRate;
		int			iItemRarity;
	...
	/*	MONSTER TYPE + DROP RATE + CHANCES TO DROP*/
	beastStruct_t	beast[] =
	{
		{ "monster_terrorist", 100, 100 },
		{ "Goblin", 75, 75 },
		{ "Squire", 65, 55 },
		{ "ZogZog", 45, 100 },
		{ "Owl", 15, 15 },
	...
	char szItemDropped[64];
	int iBeastIndex = 0;

	for (int iBeastIndex = 0; iBeastIndex <= ARRAYSIZE(beast); iBeastIndex++)
	{
		if ((!strcmp((char*)beast[iBeastIndex].sName, szEnemyType)))
		{
		//	ALERT(at_console, "found %s in beast, index is %i\n", szEnemyType, iBeastIndex);
		}
	}

	for (int i = 0; i < 1; i++)
	{
		int BeastType = iBeastIndex;
		int *DropChance;
		int *Rarity;

		//	BeastType = RANDOM_LONG(0, 5); //Since we have 6 beasts in our array
		Rarity = (int*)beast[BeastType].iItemRarity;  //Get the rarity of items the beast should drop from the aBeast array
		DropChance = (int*)RANDOM_LONG(0, 100); //Picks a number between 0 and 100)
		vector<int> aTemp;//A temporary array that will allow us what item type to drop

	//	ALERT(at_console, "%i_ %s is slain", i, (char*)beast[BeastType].sName);

		if (DropChance > (int*)beast[BeastType].iDropRate)//The DropChance is bigger than the droprate for this beast
		{
		//	ALERT(at_console, ".\n");//We stop here, this beast is considered to not have dropped an item.
		}
		else//On the other hand, DropChance is less or equal the droprate for this beast
		{
		//	ALERT(at_console, " dropping "); //We will put some text to display what item was dropped

			aTemp.clear(); //We clear/clean the aTemp array in which we will push entries to determine what item type to drop

			for (int a = 0; a <= ARRAYSIZE(drops); a++)//We will loop through every elements of the aDrop array
			{
				if ((int*)drops[a].iItemRarity <= Rarity) //When the item drop rate is greater or equal the expected Rarity
				{
					//Picks a number between 0 and item rarity
					int *rarityChance = (int*)RANDOM_LONG(0, 100);

					if (rarityChance <= (int*)drops[a].iItemRarity)
					{
						aTemp.push_back(a);//We put the current index in the temp array. We know that this index is a possible item type to drop
					//	ALERT(at_console, "[%s]", (char*)drops[a].sName);
					}
				}
			}

			int DropType = aTemp.at(RANDOM_LONG(0, aTemp.size()-1)); //The DropType is one of the indexes contained in the temporary array
		//	ALERT(at_console, "%s.\n", (char*)drops[DropType].sName);//We display the item name that was dropped

			strcpy(szItemDropped, (char*)drops[DropType].sName);
		}
	}

	if (szItemDropped != NULL)
	{
		//	ALERT(at_console, "returning %s\n", szItemDropped);

		if (!strcmp(szItemDropped, "epipen"))
		{
			GiveNamedItem("item_epipen");
			...
			if (gmsgAmmoPickup)  // make sure the ammo messages have been linked first
			{
				// Send the message that ammo has been picked up
				MESSAGE_BEGIN(MSG_ONE, gmsgAmmoPickup, NULL, pev);
				WRITE_BYTE(0);		// ammo ID
				WRITE_BYTE(1);		// amount
				WRITE_STRING("epipen"); // buz: send also ammo name to display in hud..
				MESSAGE_END();
			}
		}

		if (!strcmp(szItemDropped, "grenade"))
		{
			GiveNamedItem("item_grenade");
			...
			if (gmsgAmmoPickup)  // make sure the ammo messages have been linked first
			{
				...
				WRITE_BYTE(0);		// ammo ID
				WRITE_BYTE(1);		// amount
				WRITE_STRING("handgrenade"); // buz: send also ammo name to display in hud..
				MESSAGE_END();
			}
		}

		//give ammo wont work for special items such as epipen or grenade
		if (GiveAmmo(RANDOM_LONG(0, 18), szItemDropped, 999) != -1)
		{
		//	ALERT(at_console, "added ammo!\n");

			EMIT_SOUND_FMOD_3D(ENT(pev), "items/9mmclip1.wav", 512.0f, -1.0f, entindex());
		}

		return szItemDropped;
	}
	else
	{
		ALERT(at_console, "loot gave nothing!\n");
