...
int CBreakable :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
	...
	// Shepard : If the breakable only takes damage from the semtex, then make sure only the semtex damages it
	if ( FBitSet( pev->spawnflags, SF_BREAK_SEMTEX_ONLY ) )
	{
		...
		if ( pEntity )
		{
			if ( pEntity->IsPlayer() )
			{
				...
				{
					if ( pPlayer->m_pActiveItem->m_iId == WEAPON_SEMTEX )
						flDamage = pev->health;
					else
						flDamage = 0;
				}
